...
static void* chunk_thread(void *arg) {
	...
	while (1) {
		...
		sds filename = sdsnew((const unsigned char*)c->data);
		int is_binary = endswith(filename, ".binary");
		...
		while (1) {
			...
			if (!is_binary) {
				chunk_size += alignment_function[destor.chunking_type](leftbuf + leftoff, leftlen, chunk_size);
			}
