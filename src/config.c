...
void load_config_from_string(sds config) {
	...
	for (i = 0; i < totlines; i++) {
		...
		} else if (strcasecmp(argv[0], "index-directory") == 0 && argc == 2) {
			destor.index_directory = sdscpy(destor.index_directory,
					argv[1]);
			destor.index_directory = sdscat(destor.index_directory, "/");
		} else if (strcasecmp(argv[0], "reverse-directory") == 0 && argc == 2) {
			destor.reverse_directory = sdscpy(destor.reverse_directory,
					argv[1]);
			destor.reverse_directory = sdscat(destor.reverse_directory, "/");
		...
		&& argc == 2) {
			...
			} else if (strcasecmp(argv[1], "whitespace") == 0) {
				destor.chunking_type = WHITESPACE;
			} else if (strcasecmp(argv[1], "whitespace-reversed") == 0) {
				destor.chunking_type = WHITESPACE_REVERSED;
			} else if (strcasecmp(argv[1], "evenbyte") == 0) {
				destor.chunking_type = EVENBYTE;
			} else if (strcasecmp(argv[1], "fourbyte") == 0) {
				destor.chunking_type = FOURBYTE;
			} else if (strcasecmp(argv[1], "eightbyte") == 0) {
				destor.chunking_type = EIGHTBYTE;
			}
		} else if (strcasecmp(argv[0], "reverse-mapping") == 0
		&& argc == 2) {
			if (strcasecmp(argv[1], "lucene") == 0) {
				destor.reverse_mapping_type = LUCENE_REVERSE_MAPPING;
			} else if (strcasecmp(argv[1], "external") == 0) {
				destor.reverse_mapping_type = EXTERNAL_REVERSE_MAPPING;
			} else if (strcasecmp(argv[1], "optimized-external") == 0) {
				destor.reverse_mapping_type = READ_OPTIMIZED_EXTERNAL_REVERSE_MAPPING;
			} else if (strcasecmp(argv[1], "exhaustive") == 0) {
				destor.reverse_mapping_type = EXHAUSTIVE_SEARCH_REVERSE_MAPPING;
			} else if (strcasecmp(argv[1], "db") == 0) {
				destor.reverse_mapping_type = DB_REVERSE_MAPPING;
				destor.is_textual_doc_id = false;
			} else if (strcasecmp(argv[1], "in-doc") == 0) {
				destor.reverse_mapping_type = IN_DOC_REVERSE_MAPPING;
			}
		} else if (strcasecmp(argv[0], "indirect-path-strings") == 0
		&& argc == 2) {
			if (strcasecmp(argv[1], "yes") == 0) {
				destor.is_path_indirected = true;
			} else if (strcasecmp(argv[1], "no") == 0) {
				destor.is_path_indirected = false;
			}
		} else if (strcasecmp(argv[0], "threaded-lookup") == 0
		&& argc == 2) {
			if (strcasecmp(argv[1], "yes") == 0) {
				destor.is_threaded_lookup = true;
			} else if (strcasecmp(argv[1], "no") == 0) {
				destor.is_threaded_lookup = false;
			}
		} else if (strcasecmp(argv[0], "textual-doc-id") == 0
		&& argc == 2) {
			if (strcasecmp(argv[1], "yes") == 0) {
		...
		} else if (strcasecmp(argv[0], "offsets-mode") == 0
		&& argc == 2) {
			if (strcasecmp(argv[1], "none") == 0) {
				destor.offsets_mode = NO_OFFSETS;
			} else if (strcasecmp(argv[1], "global") == 0) {
				destor.offsets_mode = GLOBAL_OFFSETS;
			} else if (strcasecmp(argv[1], "term-vectors") == 0) {
				destor.offsets_mode = TERM_VECTOR_OFFSETS;
			} else if (strcasecmp(argv[1], "global-term-vectors") == 0) {
				destor.offsets_mode = GLOBAL_TERM_VECTOR_OFFSETS;
			}
		} else if (strcasecmp(argv[0], "tf-idf") == 0
		&& argc == 2) {
			if (strcasecmp(argv[1], "yes") == 0) {
				destor.tf_idf = 1;
			} else if (strcasecmp(argv[1], "no") == 0) {
		...
		if (destor.offsets_mode > GLOBAL_OFFSETS && destor.tf_idf) {
			puts("tf-idf mode is incompatible with the set offsets mode\n");
			goto loaderr;
		}
