...
struct backupVersion* create_backup_version(const char *path) {
	...
	if ((b->metadata_fp = fopen(fname, "w")) == 0) {
		...
		fprintf(stderr, "file: %s, Error code: %d\n", fname, errno);
		exit(1);
	}
	...
	fwrite(&b->indexed, sizeof(b->indexed), 1, b->metadata_fp);
...
struct backupVersion* open_backup_version(int number) {
	...
	fread(&b->indexed, sizeof(b->indexed), 1, b->metadata_fp);
...
struct fileRecipeMeta* read_next_file_recipe_meta(struct backupVersion* b, bool should_reset) {
